      ...
      switch (action) {
        ...
        case 'generate_images': {
          ...
          try {
            ...
            console.log(`All ${results.length} images generated, storing to Firebase Storage...`);
            ...
            const uploadPromises = results.map(async ({ mediaUrl, index }) => {
              ...
              if (!parsed) {
                throw new Error(`Failed to parse image data URL for image ${index + 1}`);
              }
              ...
              console.log(`Stored image ${index + 1} to ${fileName} with URL: ${publicUrl}`);
            ...
            console.log(`Successfully stored ${uploadedImages.length} images to Firebase Storage`);
