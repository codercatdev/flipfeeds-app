FlipFeeds: Core Firestore Data SchemaThis schema is designed for a multi-tenant system where "Feeds" are the tenants. All paths should be constants in your code.We will use a version field (e.g., v1) on top-level collections to make future schema migrations easier.1. Collection: usersStores public-facing user profile data and private user-specific info.Path: users/{userId}{userId} will be the Firebase Auth UID.Document ({userId}):{
  "uid": "fbas_auth_uid",
...
```json
{
  ...
  "ownerId": "userId_of_owner", // Ref to users/{userId}
  "stats": {
    "memberCount": 150,
    "flipCount": 45
  },
...
3. Sub-Collection: feedMembersTracks the relationship between users and feeds. This is critical for security rules.Path: feeds/{feedId}/members/{userId}This structure allows us to easily check if a user is in a feed.Document ({userId}):{
...
**User Reference:** `users/{userId}/personalFeed`
...
**Path:** `flips/{flipId}/comments/{commentId}`
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    function getRole(feedId) {
      return get(/databases/$(database)/documents/feeds/$(feedId)/members/$(request.auth.uid)).data.role;
    }
