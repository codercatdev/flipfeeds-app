...
describe('feedFlows', () => {
  ...
  describe('createFeedFlow', () => {
    ...
    afterEach(async () => {
      ...
      const userFeedQuery = await db.collection('users').doc(testUid).collection('feeds').get();
    ...
    it('should create a new feed', async () => {
      ...
      const memberDoc = await db.collection('feeds').doc(feedId).collection('members').doc(testUid).get();
      expect(memberDoc.exists).to.be.true;
      expect(memberDoc.data()).to.have.property('role', 'admin');

      const userFeedDoc = await db.collection('users').doc(testUid).collection('feeds').doc(feedId).get();
